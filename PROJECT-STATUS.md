...
| 5 | Website: Cloudflare Pages + React + domain + www redirect + HTTPS | ✅ **Rebuilt 2026-09-11.** Single-page site (`HNK/site/`), reskinned from a proven Kimi Agent template (formerly "Peninsula Tech"): GSAP scroll reveals, scroll-reactive glitch service gallery, a three.js shader panel, Formspree-backed enquiry form (`https://formspree.io/f/mdeojlll`, falls back to mailto if unset), fully fluid `clamp()` sizing end to end. Live at https://hnkiot.com, replacing the earlier 17-page `Web/` app (kept on disk, not deployed). TLS cert (Google Trust Services, auto-renew), http to https 301, www to apex 301 (path and query preserved), SSL Full (strict), security headers all still in place. DNSSEC: confirm status. |
